...
jest.mock('fs/promises', () => ({
  ...
  readFile: async (file: string, _enc?: any) => {
    ...
    if (v === undefined) {
      const err: any = new Error(`ENOENT: no such file or directory, open '${file}'`);
      err.code = 'ENOENT';
      throw err;
    }
    return v;
...
describe('KeyService', () => {
  ...
  afterEach(async () => {
    try {
      ...
      try { inMemoryStore.clear(); } catch (e) {}
    } catch (e) {
      // ignore
    }
  });
  ...
  test('storeEncrypt and decryptKey roundtrip', async () => {
    ...
    const res = await ks.storeEncrypt(network, currency, type, privateKey, mnemonic);
