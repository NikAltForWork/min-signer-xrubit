...
jest.mock('tronweb', () => {
  return jest.fn().mockImplementation((opts) => {
    return {
      toSun: (v: any) => v,
      address: { fromPrivateKey: () => (opts && opts.privateKey) || 'TTESTADDRESS' },
      transactionBuilder: { triggerSmartContract: jest.fn().mockResolvedValue({ transaction: { raw_data: {}, raw_data_hex: 'aa' }, txid: 'TXID' }) },
      trx: { sign: jest.fn().mockResolvedValue('SIGNED'), sendRawTransaction: jest.fn().mockResolvedValue(true) },
      isAddress: () => true,
    };
  });
});
...
describe('USDTService (unit)', () => {
  ...
  const svc = new USDTService('testpk', resourceQ as any, balanceQ as any, activationQ as any);
  ...
  test('getLastTransaction: returns tx id when present, else null', async () => {
    // mock global fetch
    (global as any).fetch = jest.fn().mockResolvedValue({ json: async () => ({ data: [ { transaction_id: 'tx123' } ] }) });
  ...
  test('getBalance: reads trc20 balance and converts to human value', async () => {
    const contract = svc.getContract();
    (global as any).fetch = jest.fn().mockResolvedValue({ json: async () => ({ data: [ { trc20: [ { [contract]: 2000000 } ] } ] }) });
  ...
  test('finishControlledTransaction returns early when wallet key missing', async () => {
    ...
    const res = await svc.finishControlledTransaction({ address: 'TABC', balance: '1', id: '1' });
    expect(res).toBeUndefined();
  });

  test('finishFiatToCryptoTransaction signs and notifies', async () => {
    // prepare transaction result on triggerSmartContract
    (svc as any).tronWeb.transactionBuilder.triggerSmartContract = jest.fn().mockResolvedValue({ transaction: { raw_data: {} }, txid: 'TX123' });
    (svc as any).tronWeb.trx.sign = jest.fn().mockResolvedValue('SIGNED_TX');
    (svc as any).tronWeb.trx.sendRawTransaction = jest.fn().mockResolvedValue(true);

    const spyNotify = (svc as any).notifier.notifyStatus;

    await svc.finishFiatToCryptoTransaction({ network: 'TRC20', currency: 'USDTTRC20', type: 'big', id: 'id1', to: 'TOADDR', amount: '1', callback: 'cb' });

    expect((svc as any).tronWeb.transactionBuilder.triggerSmartContract).toHaveBeenCalled();
    expect((svc as any).tronWeb.trx.sign).toHaveBeenCalled();
    expect(spyNotify).toHaveBeenCalledWith(expect.objectContaining({ callback: 'cb', id: 'id1', tx_id: 'TX123' }));
  });
  ...
  test('finishActivationControl requests resources and enqueues resource job', async () => {
    ...
    await svc.finishActivationControl({ network: 'TRC20', currency: 'USDTTRC20', type: 'big', to: 'TO', amount: '1', id: 'act1', isCryptoToFiat: false, callback: 'cb' });
    ...
    expect(addJobSpy).toHaveBeenCalledWith(expect.objectContaining({ id: 'act1', wallet: expect.any(String) }), 'act1', expect.any(Number));
  });
  ...
  test('finishControlledTransaction performs transfer when key present', async () => {
    ...
    mockedTron.mockImplementationOnce((opts) => ({
      transactionBuilder: { triggerSmartContract: jest.fn().mockResolvedValue({ transaction: { raw_data: {}, raw_data_hex: 'aa' }, txid: 'TX_OK' }) },
      trx: { sign: jest.fn().mockResolvedValue('SIG'), sendRawTransaction: jest.fn().mockResolvedValue(true) },
      address: { fromPrivateKey: () => 'MAINADDR' },
      toSun: (v) => v,
      isAddress: () => true,
    }));

    const res = await svc.finishControlledTransaction({ address: 'TADDR', balance: '1', id: 'tx1' });
