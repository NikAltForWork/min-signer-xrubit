...
describe('CryptoServiceFactory', () => {
  ...
  const factory = new CryptoServiceFactory(balanceQ as any, resourceQ as any, activationQ as any);
  ...
  test('throws on unsupported', async () => {
    await expect(factory.createCryptoService('FOO', 'BAR', 'big')).rejects.toThrow(/Unsupported network\/currency/);
  });
});
